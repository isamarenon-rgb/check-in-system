...
window.selectGrade = (g) => {
    ...
    tempCheck = {}; // ล้างค่าเก่าก่อนเช็คใหม่
    renderList();
...
window.viewAbsenteeReport = async () => {
    ...
    container.innerHTML = `
        <div style="margin-bottom:15px;">
            <button onclick="goBack()" style="width:100%; padding:12px; background:#4a4a4a; color:white; border:none; border-radius:12px; cursor:pointer; font-weight:bold;">⬅️ ย้อนกลับ</button>
        </div>
    ...
    try {
        ...
        if (logs.length === 0) {
            reportContent.innerHTML = '<p style="text-align:center; color:#888;">📅 วันนี้ยังไม่มีข้อมูล</p>';
            return;
        }
    ...
    } catch (e) { reportContent.innerHTML = '<div style="color:red;">⚠️ โหลดข้อมูลไม่สำเร็จ</div>'; }
};

// --- หน้าแอดมิน / คลังสำรอง ---
window.renderAdmin = async () => {
    const container = document.getElementById('masterRecdList');
    container.innerHTML = '<div style="text-align:center; padding:20px;">กำลังโหลดคลัง...</div>';
    ...
    try {
        ...
        container.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:10px; margin-bottom:20px;">
                <button onclick="handleLogout()" style="width:100%; padding:12px; background:#eee; color:#333; border:none; border-radius:12px; cursor:pointer;">🏠 กลับหน้าล็อกอิน</button>
                <button onclick="downloadExcel()" style="width:100%; padding:12px; background:#2e7d32; color:white; border:none; border-radius:12px; cursor:pointer;">💾 โหลด Excel ทั้งหมด</button>
            </div>
        ...
        if (data.length === 0) {
            archiveContent.innerHTML = '<p>คลังว่างเปล่า</p>';
            return;
        }

        data.slice().reverse().forEach(h => {
            const d = document.createElement('div');
            d.style.cssText = "background:white; padding:12px; border-radius:12px; margin-bottom:8px; border-left:5px solid #7c4dff; box-shadow:0 2px 6px rgba(0,0,0,0.05);";
            d.innerHTML = `
                <div style="display:flex; justify-content:space-between;">
                    <strong>ห้อง ${h.grade}</strong>
                    <span>📅 ${h.date}</span>
                </div>
                <div style="color:red; margin-top:4px; font-size:0.8rem;">ขาด: ${h.a || '-'}</div>
            `;
            archiveContent.appendChild(d);
        });
    } catch (e) { container.innerHTML = '<div style="color:red;">⚠️ เกิดข้อผิดพลาด</div>'; }
};
...
function applyUI() {
    ...
    if(l) { const el = document.getElementById('appLogo'); if(el) el.src = l; }
    if(b) { const el = document.getElementById('bgOverlay'); if(el) el.style.backgroundImage = `url(${b})`; }
}
applyUI();
